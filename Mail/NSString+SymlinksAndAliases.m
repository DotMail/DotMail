...
- (NSString *)stringByConditionallyResolvingAlias
{
	...
	if (url != NULL)
	{
		...
		{
			CFErrorRef err;
			CFDataRef bookmarkDataRef = CFURLCreateBookmarkDataFromFile(kCFAllocatorDefault, url, &err);
			if (err == NULL)
			{
				Boolean wasAliased;
				CFURLRef resolvedUrl = CFURLCreateByResolvingBookmarkData(kCFAllocatorDefault, bookmarkDataRef, kCFBookmarkResolutionWithoutUIMask, NULL, 0, &wasAliased, &err);
				if (resolvedUrl != NULL)
				{
					resolvedPath =
